...
<script>
import { exclude, findAll } from "../service/produtos.service";
    ...
    excluir(model) {
      if (confirm(`Confirma exclusão de ${model.nome}?`)) {
        exclude(model.id)
          .then(
            () =>
              (this.produtos = this.produtos.filter((c) => c.id !== model.id))
          )
          .catch(() => alert("Erro ao excluir. O produto está em uso."));
      }
    },
  },
  mounted() {
    findAll().then((resp) => (this.produtos = resp.data));
  },
};
</script>
